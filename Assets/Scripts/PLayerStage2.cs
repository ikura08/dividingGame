...
public class PLayerStage2 : MonoBehaviour, IJumpable  //ここでふたつのinterfaceを適用してる
{
    [SerializeField] private Stage2Scr cameraMove;
    ...
    //     while (true)
    //     {
    //         if (direction.x > 0) //右に動くアニメーション
    //             playerAnimationScr.currentState = 1;
    //         else if (direction.x < 0) //左に動くアニメーション
    //             playerAnimationScr.currentState = -1;
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        if (other.gameObject == firstArea)
            cameraMove.SetArea(1);

        if (other.gameObject == secondArea)
            cameraMove.SetArea(2);

        if (other.gameObject == thirdArea)
            cameraMove.SetArea(3);
    }
}
